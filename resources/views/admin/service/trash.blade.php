    ...
    <div class="container mx-auto max-w-7xl">
        <div class="flex justify-between items-center mb-6">
            <div class="flex items-center gap-4">
                ...
                <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Trashed Services</h1>
            </div>
        </div>
                    ...
                    <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                        @forelse($services as $service)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition">
                                ...
                                <td class="px-6 py-4">
                                    {{ $service->fee ? '$' . number_format($service->fee, 2) : 'Free' }}
                                </td>
                                <td class="px-6 py-4 text-red-500">
                                    {{ $service->deleted_at->diffForHumans() }}
                                </td>
                                <td class="px-6 py-4 text-right space-x-2">
                                    <a href="{{ route('admin.service.restore', $service->service_id) }}"
                                        class="text-emerald-600 hover:text-emerald-800" title="Restore">
                                        <i class="fa-solid fa-rotate-left"></i> Restore
                                    </a>
                                    <a href="{{ route('admin.service.forceDelete', $service->service_id) }}"
