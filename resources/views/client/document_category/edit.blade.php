...
@section('content')
    <div class="container mx-auto max-w-4xl">
        <div class="flex items-center gap-4 mb-6">
            ...
            <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Edit Category</h1>
        </div>

        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
            <form action="{{ route('client.category.update', $category->document_categorie_id) }}" method="POST"
                class="space-y-6">
                @csrf
                {{-- No PUT directive as controller uses stored route POST definition --}}
                ...
                <div>
                    ...
                    <textarea name="description" rows="4" required
                        class="w-full rounded-lg border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-200 focus:ring-blue-500 focus:border-blue-500">{{ old('description', $category->description) }}</textarea>
