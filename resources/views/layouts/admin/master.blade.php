...
<body x-data="{
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-white
            ...
            <nav class="px-4 py-6 space-y-2 text-base font-semibold">
                <a href="{{ route('admin.dashboard') }}" @click="sidebarOpen=false" class="flex items-center gap-3 px-4 py-3 rounded-xl
    ...
    : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                ...
                <div x-data="{ open: {{ request()->routeIs('admin.tasks.*') ? 'true' : 'false' }} }">
                    <button @click="open = !open"
                        class="w-full flex items-center justify-between px-4 py-3 rounded-xl transition-colors duration-200
                        {{ request()->routeIs('admin.tasks.*') ? 'text-white bg-slate-800' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                        <div class="flex items-center gap-3">
                    ...
                    <div x-show="open" x-cloak class="pl-12 pr-2 py-2 space-y-1">
                        <a href="{{ route('admin.tasks.index') }}"
                            class="block py-2 text-sm hover:text-white {{ request()->routeIs('admin.tasks.index') ? 'text-blue-400' : 'text-slate-400' }}">
                            All Tasks
                        </a>
                        <a href="{{ route('admin.tasks.create') }}"
                            class="block py-2 text-sm hover:text-white {{ request()->routeIs('admin.tasks.create') ? 'text-blue-400' : 'text-slate-400' }}">
                            Add New
                        </a>
                        <a href="{{ route('admin.tasks.trash') }}"
                            class="block py-2 text-sm hover:text-white {{ request()->routeIs('admin.tasks.trash') ? 'text-blue-400' : 'text-slate-400' }}">
                            Trash
                        </a>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between h-16 px-6">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="hidden md:flex relative">
                            ...
                            <input type="text" placeholder="Search..." class="pl-10 pr-4 py-2 rounded-lg
                    ...
                    <div class="flex items-center gap-5 text-base font-medium">
                        ...
                        <!-- <div x-data="{ open:false }" class="relative">
                            ...
                            <div x-show="open" x-cloak @click.away="open=false"
                                 class="absolute right-0 mt-3 w-40 z-50
                                        bg-white dark:bg-slate-800
                                        rounded-xl shadow-xl border dark:border-slate-700">
                                <ul class="py-2 text-sm text-slate-700 dark:text-slate-200">
                                    <li>
                                        <button onclick="changeLanguage('en')" class="w-full text-left px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-700">
                                            English
                                        </button>
                                    </li>
                                    <li>
                                        <button onclick="changeLanguage('hi')" class="w-full text-left px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-700">
                                            Hindi (हिंदी)
                                        </button>
                                    </li>
                                    <li>
                                        <button onclick="changeLanguage('mr')" class="w-full text-left px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-700">
                                            Marathi (मराठी)
                                        </button>
                                    </li>
                                </ul>
